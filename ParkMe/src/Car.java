...
public class Car {
	...
	public void move() {
		...
		if (steer == 0) {
			...
			double ym = m_width / 2 + m_axleDist / Math.tan(steer);
	...
	public void display(GL2 gl) {
		...
		drawWheel(gl, 0, - m_width / 2, 0);
		drawWheel(gl, m_length - m_axleDist * 2, m_width / 2, Math.toRadians(m_steer));
		double ym = m_width / 2 + m_axleDist / Math.tan(Math.toRadians(m_steer));
	...
	private void drawWheel(GL2 gl, double x, double y, double s) {
		...
		gl.glRotated(Math.toDegrees(s), 0, 0, 1);
		gl.glPolygonMode(GL2.GL_FRONT_AND_BACK, GL2.GL_LINE);
		gl.glBegin(GL2.GL_POLYGON);
